...
export default function getClasses(variables, classes) {
  ...
  Object.keys(THEME_COLORS).forEach((item) => {
    ...
    // custom
    _classes['text' + classColor + 'Light'] = {color: colorLevel(variables, THEME_COLORS[item], -9)};
    _classes['text' + classColor + 'Dark'] = {color: colorLevel(variables, THEME_COLORS[item], 9)};

    // custom / experimental
    _classes['text' + classColor + 'Lightest'] = {color: colorLevel(variables, THEME_COLORS[item], -11)};
    _classes['text' + classColor + 'Darkest'] = {color: colorLevel(variables, THEME_COLORS[item], -11)};
  });
